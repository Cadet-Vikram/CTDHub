...
class FaceDetector:
    ...
    def load(self):
        try:
            ...
            logger.warning(f"  MTCNN not available ({e}) — using mock detector")
            self._model = None
...
class EmbeddingExtractor:
    ...
    def load(self):
        # Try ArcFace (insightface) first
        try:
            from deepface import DeepFace
            #Warm up - downloads wieghts on first call
            import numpy as np
            ...
            logger.warning(f"  DeepFace not available: {e} - mock mode")
            self._model = None
    ...
    def extract(self, face_image: np.ndarray) -> np.ndarray:
        if self._model == "deepface":
            try:
                from deepface import DeepFace
                result = DeepFace.represent(face_image, model_name="ArcFace", enforce_detection=True, detector_backend="opencv")
                emb = np.array(result[0]["embedding"], dtype=np.float32)
                return emb / (np.linalg.norm(emb) + 1e-10)  # Normalize to unit length
            except Exception as e:
                logger.warning(f"  DeepFace extract failed: {e}")
...
class SimilarityMatcher:
    """Cosine similarity matching against an in-memory database"""
    ...
    def search(
        ...
        scores = [
            {"child_id": cid, "similarity": self.cosine(query, emb),

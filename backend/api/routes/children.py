...
def _decode_image(content: bytes):
    """Decode bytes → numpy array via cv2 or Pillow fallback"""
    try:
        import cv2
...
async def register_child(
    ...
    if photo:
        ...
        # Extract embedding only for children ≥ 5
        if age >= 5:
            image = _decode_image(content)
            if image is not None:
                face_model = getattr(request.app.state, "face_model", None)
                if face_model:
                    embedding, _ = face_model.process_image(image)
                    if embedding is not None:
                        embedding_json = face_model.to_json(embedding)
...
async def update_status(
    child_id: str, status: str, db: AsyncSession = Depends(get_db)
):
    result = await db.execute(select(Child).where(Child.id == child_id))
    child  = result.scalar_one_or_none()
    if not child:
